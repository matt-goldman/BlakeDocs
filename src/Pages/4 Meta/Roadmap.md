...
Blake has reached a stable, production-ready state. The core static site generation functionality is complete and in use across multiple projects.
...
- **BlakePlugin.RSS** - Automatic feed creation for blog-style content
- **BlakePlugin.OpenGraph** - Generate meta tags for SEO and social preview
...
Blake generates static sites. A Blazor project can be configured to enable pre-rendering and mixed-mode interactivity; that belongs in a template, not in Blake itself.
...
Blake generates Blazor applications. Supporting other output formats would compromise the tight integration with the Blazor ecosystem that makes Blake powerful.
...
Blake is still in active development following a stable 1.0 release. The core functionality is production-ready, but APIs may change based on community feedback.
:::
